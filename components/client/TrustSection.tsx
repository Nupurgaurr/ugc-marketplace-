  ...
  return (
    <section className="section section--ruled section--band" id="vetting">
      <div className="container" ref={ref}>
        <SectionEyebrow>Vetting</SectionEyebrow>
        ...
        <div className={styles.pipeline}>
          {PIPELINE.map((node, i) => (
            <span key={node} style={{ display: 'contents' }}>
              <span className={`${styles.node} ${i === PIPELINE.length - 1 ? styles.nodeLive : ''}`}>{node}</span>
              {i < PIPELINE.length - 1 && (
        ...
        <p className="body-dim" style={{ fontSize: '0.9rem', maxWidth: '52ch' }}>
          Only creators at the final stage are visible to clients. Everything before it sits in
        ...
        <p className={styles.credit}>
          Operated by <b>Blackcoffee Media</b> — six years, 250+ brands, 30+ specialists across
          India, running performance, creative and retention end to end.
        </p>
      </div>
    </section>
  );
